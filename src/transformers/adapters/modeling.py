...
class Adapter(nn.Module):
    ...
    def forward(self, x, residual_input, output_gating=False):
        ...
        # activation_matching
        # activation_matching_path_base = 'activation_matching_permutation_activations/rte_rte_pfeiffer_activations/'
        # activation_matching_files = os.listdir(activation_matching_path_base)   # 读入文件夹
        # activation_matching_number = len(activation_matching_files)
        # activation_matching_path=os.path.join(activation_matching_path_base, str(activation_matching_number)+".pt")
        # torch.save(down, activation_matching_path)
        # breakpoint()
...
class BertFusion(nn.Module):
    ...
    def forward(self, query, key, value, residual, output_attentions: bool = False):
        # breakpoint()
        if self.config["model_lambda"]:
            model_lambda=self.config["model_lambda"]
            model_lambda=[ml/sum(model_lambda) for ml in model_lambda]
            context_layer=0
            # context_layer=key[:,:,0,:]
            for klindex in range(key.size(2)):
                context_layer+=key[:,:,klindex,:]*model_lambda[klindex]
            # context_layer=torch.squeeze(context_layer, dim=2)
        ...
        if self.config["query"]:# and not self.config["model_lambda"]:
            query_layer = self.query(query)
        else:
            query_layer = query

        if self.config["key"]:# and not self.config["model_lambda"]:
            key_layer = self.key(key)
        else:
            key_layer = key

        if self.config["value"] and self.config["value_before_softmax"]:# and not self.config["model_lambda"]:
        ...
        if self.config["value"] and not self.config["value_before_softmax"]:# and not self.config["model_lambda"]:
            # key/value have dims => batch, toks, number-of-adapters, feats

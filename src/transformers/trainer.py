...
class Trainer:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # breakpoint()
        # print("here one model pass forward")
    ...
    def fisher_compute(
            ...
            fisher_save_path: str = "",
            ignore_keys: Optional[List[str]] = None,
            metric_key_prefix: str = "eval",
        ):

        # TODO: make a proper dataloader
        fisher_dataloader = self.get_eval_dataloader(dataset)

        fishers = self.fisher_compute_loop(
            fisher_dataloader,
            description="Fisher Computing",
        ...
        # breakpoint()
        fisher_dict=OrderedDict(zip(merge_variables, fishers))
        torch.save(fisher_dict, fisher_save_path)
        return fisher_dict

    def fisher_compute_loop(
        self,
        ...
        # TODO: MAYBE have some bad impact
        model.eval()

        # TODO: eva_dataset need to be change
        ...
        # TODO: EDIT these posses, preds and lables
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            # TODO: fisher_compute_step()
            # we need to calculate grads and get them back.
            # we don't need loss and labels.
            # loss, logits, labels,
            fishers = self.fisher_compute_step(
                model = model, 
                inputs = inputs, 
                merge_variables = merge_variables, 
        ...
        # breakpoint()
        #put fishers_ls into all_fishers
        all_fishers = [sum(g) for g in zip(*fishers_ls)]
        for index, fisher in enumerate(all_fishers):
            all_fishers[index]=torch.div(fisher,observed_num_examples)
        return all_fishers
        ...
        return NamedTuple(predictions=all_preds, label_ids=all_labels, grads=all_losses, num_samples=num_samples)

    def fisher_compute_step(
            self,
            model: nn.Module,
            inputs: Dict[str, Union[torch.Tensor, Any]],
        ...
        has_labels = all(inputs.get(k) is not None for k in self.label_names) # True
        inputs = self._prepare_inputs(inputs)
        ...
        if has_labels:
            ...
            labels = None
        # breakpoint()
        # HERE we drop torch.no_grad
        if is_sagemaker_mp_enabled():
	    # 没用上：smp_f_b的返回是loss_mb, 但我们还需要logits. 可能需要重写smp_f_b
            # raw_outputs = smp_forward_backward(model, inputs, gradient_accumulation_steps=1)
        ...
        fisher_dict=OrderedDict(zip(merge_variables,len(merge_variables)*[0]))
        for i in range(log_probs.shape[0]):
            for j in range(log_probs.shape[1]):
                log_probs[i][j].backward(retain_graph=True)
                for k,v in model.named_parameters():
                    if k in merge_variables:
                        fisher_dict[k]=fisher_dict[k]+probs[i][j]*torch.square(v.grad)
                        # v.grad.data.zero_()
                model.zero_grad()

        fishers=list(fisher_dict.values())
        # breakpoint()
        return fishers
    ...
    def evaluation_loop(
        ...
        # 和dataloader打包在一起，接下来就要正式train了
        # 需要跑，有一些并行化操作有必要（基本上是正常返回
        model = self._wrap_model(self.model, training=False, dataloader=dataloader)
